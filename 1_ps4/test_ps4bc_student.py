...
class TestPS4BC(unittest.TestCase):
    ...
    def test_plaintext_message_generate_pad(self):
        ...
        acceptable = set([x for x in range(0,110)])
        p = student.PlaintextMessage("a"*1500)
        r = p.generate_pad()
        for c in r:
            self.assertTrue(c in acceptable, f"Number in generated pad {c} not acceptable for expected range.")
    ...
    def test_plaintext_message_pad(self):
        ...
        for p, init_pad, set_pad in pds:
            result = p.get_pad()
            self.assertEqual(
                init_pad, result, f"The PlaintextMessage was initialized with the pad {init_pad} but get_pad() returned {result}. Did you save pad as a copy?")
            result[0] = -999
            result = p.get_pad()
            self.assertEqual(
                init_pad, result, f"The PlaintextMessage was initialized with the pad {init_pad} but get_pad() returned {result} after the result of get_pad() was mutated. Did you return a copy of the pad?")

            mut_pad = set_pad.copy()
            p.change_pad(mut_pad)
            mut_pad[0] = -999
            result = p.get_pad()
            self.assertEqual(
                set_pad, result, f"The PlaintextMessage pad was changed to {set_pad} but get_pad() returned {result}. Did you save pad as a copy?")
