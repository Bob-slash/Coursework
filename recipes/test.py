...
example_recipes = [
    ('compound', 'chili', [('beans', 3), ('cheese', 10),
     ('chili powder', 1), ('cornbread', 2), ('protein', 1)]),
    ('atomic', 'beans', 5),
    ('compound', 'cornbread', [
     ('cornmeal', 3), ('milk', 1), ('butter', 5), ('salt', 1), ('flour', 2)]),
    ('atomic', 'cornmeal', 7.5),
    ('compound', 'burger', [('bread', 2), ('cheese', 1),
     ('lettuce', 1), ('protein', 1), ('ketchup', 1)]),
    ('compound', 'burger', [('bread', 2),
     ('cheese', 2), ('lettuce', 1), ('protein', 2),]),
    ('atomic', 'lettuce', 2),
    ...
    ('compound', 'ketchup', [('tomato', 30), ('vinegar', 3),
     ('salt', 1), ('sugar', 2), ('cinnamon', 1)]),  # the fancy ketchup
    ('atomic', 'cow', 100),
...
def compare_recipe_list(expected, result):
    assert len(expected) == len(
        result), f'Expected recipes list of length {len(expected)} but got {len(result)}'
    assert type(expected) == type(
        result), f'Expected recipes list to be of type {type(expected)} but got {type(result)}'
    for item in result:
        assert isinstance(
            item, tuple), f'Expected all items in recipes to be a tuple but got {item}'
        assert len(
            item) == 3, f'Expected all items in recipes to have length 3 but got {len(item)} \n for {item}'
        a, b, c = item
        assert isinstance(a, str) and a in {
            'atomic', 'compound'}, f'Expected first item in recipe tuple to be atomic or compound but got {a} in {item}'
        assert isinstance(
            b, str), f'Expected second item in recipe tuple to be a string but got {type(b)} for {b} in {item}'
        expected_type = (list, ) if a == 'compound' else (int, float)
        assert isinstance(
            c, expected_type), f'Expected third item in recipe to be of type {expected_type} but got {type(c)} for {c} in {item}'

    exp = set((a, b, tuple(c) if a == 'compound' else c)
...
def canonize_flat_recipe(recipe):
    ...
    if recipe is None:
        return None
    assert isinstance(
        recipe, dict), "Each recipe should be flat, e.g. a dictionary!"
...
def canonize_flat_recipes(recipes):
    ...
    """
    assert isinstance(recipes, list) and all(isinstance(i, dict)
                                             for i in recipes), "Recipes should be represented as a list of dictionaries!"
...
def check_recipe_book(result, expected):
    assert len(result) == len(
        expected), "recipe books should be the same length!"
...
def test_lowest_cost_examples_forbidden():
    ...
    assert abs(lab.lowest_cost(example_recipes,
               'cornmeal', ('cow',)) - 7.5) <= 1e-6
...
def test_scale_recipe_small(scale):
    ...
    assert len(res) == len(
        recipe), f"expected recipe of length {len(recipe)}, got recipe of length {len(res)}"
    assert res == expected
...
def test_grocery_list_random(num_ingredients):
    ...
    totals = {str(i): random.randint(1, max_quantity)
    ...
    for _ in range(max_recipes-1):
        ...
        flat_recipes.append(
            {ing: total for ing, total in totals.items() if total})
...
def test_cheapest_examples_all_included():
    ...
    assert lab.cheapest_flat_recipe(example_recipes, 'cheese') == {
        'cow': 1, 'milking stool': 1, 'time': 1}

    # more complex
    print("lab", lab.cheapest_flat_recipe(example_recipes, 'burger'))
...
def test_cheapest_examples_forbidden():
    ...
    # compound food items, only one layer deep
    assert lab.cheapest_flat_recipe(
        example_recipes, 'protein', ('cow',)) is None

    # two layers
    assert lab.cheapest_flat_recipe(example_recipes, 'cheese', ('milking stool',)) == {
        'cutting-edge laboratory': 11}
    assert lab.cheapest_flat_recipe(
        example_recipes, 'cheese', ('milking stool', 'cutting-edge laboratory')) is None
...
def test_cheapest_big_all_included(testnum):
    for i in range(testnum*11, (testnum+1)*11):
        ...
        assert graph == orig_graph, "be careful not the change the input!"
        print("lab", canonize_flat_recipe(result))
        print("test", canonize_flat_recipe(test_data['orig_min_recipe']))
...
def test_cheapest_big_excluded(testnum):
    for i in range(testnum*11, (testnum+1)*11):
        ...
        for filt, expected in [
            ...
            result = lab.cheapest_flat_recipe(graph, target)
            assert graph == orig_graph, "be careful not the change the input!"
            assert canonize_flat_recipe(
                result) == canonize_flat_recipe(expected)
...
def test_cheapest_big_excluded_forbidden(testnum):
    ...
    for i in range(testnum*11, (testnum+1)*11):
        ...
        for filt, expected in [
            ...
            graph = _filter_graph(test_data['graph'], test_data[filt][::2])
            orig_graph = copy.deepcopy(graph)
            result = lab.cheapest_flat_recipe(
                graph, target, test_data[filt][1::2])
            assert graph == orig_graph, "be careful not the change the input!"
            assert canonize_flat_recipe(
                result) == canonize_flat_recipe(expected)
...
def test_all_recipes_examples_all_included():
    ...
    # two layers
    assert canonize_flat_recipes(lab.all_flat_recipes(example_recipes, 'cheese')) == canonize_flat_recipes(
        [{'cow': 1, 'milking stool': 1, 'time': 1}, {'cutting-edge laboratory': 11}])

    # more complex
    burgers = [
        {'yeast': 2, 'salt': 2, 'flour': 4, 'cow': 4,
            'milking stool': 2, 'time': 2, 'lettuce': 1},
        {'yeast': 2, 'salt': 2, 'flour': 4,
            'cutting-edge laboratory': 22, 'lettuce': 1, 'cow': 2},
        {'yeast': 2, 'salt': 3, 'flour': 4, 'cow': 2, 'milking stool': 1, 'time': 1,
            'lettuce': 1, 'tomato': 30, 'vinegar': 3, 'sugar': 2, 'cinnamon': 1},
        {'yeast': 2, 'salt': 2, 'flour': 4, 'cow': 2, 'milking stool': 1,
            'time': 1, 'lettuce': 1, 'tomato': 30, 'vinegar': 5},
        {'yeast': 2, 'salt': 3, 'flour': 4, 'cutting-edge laboratory': 11, 'lettuce': 1,
            'cow': 1, 'tomato': 30, 'vinegar': 3, 'sugar': 2, 'cinnamon': 1},
        {'yeast': 2, 'salt': 2, 'flour': 4, 'cutting-edge laboratory': 11,
            'lettuce': 1, 'cow': 1, 'tomato': 30, 'vinegar': 5}
    ]

    assert canonize_flat_recipes(lab.all_flat_recipes(
        example_recipes, 'burger')) == canonize_flat_recipes(burgers)
...
def test_all_recipes_examples_forbidden():
    ...
    assert canonize_flat_recipes(lab.all_flat_recipes(
        example_recipes, 'burger', ('milk',))) == canonize_flat_recipes(burgers2)
    ...
    for i in range(testnum*5, (testnum+1)*5):
        ...
        for filt, expected in [
            ...
            for graph, filter_ in [(_filter_graph(test_data['graph'], test_data[filt]), ()),
                ...
                assert graph == orig_graph, "be careful not the change the input!"
                assert canonize_flat_recipes(
                    result) == canonize_flat_recipes(expected)
